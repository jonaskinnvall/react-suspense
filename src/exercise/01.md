...
- Simple data fetching with React Suspense and ErrorBoundary
  - How we check if we have fetched data and are pending we throw our promise
    result so that react suspends and instead render the suspense fallback
    - This is wrapped in another promise making react try it until we have
      fetched data and then render the compnent with its data instead of the
      fallback
  - If there is an error we throw that error to let our ErrorBOundary component
    handle that instead
- How we can refactor this functionality into a util function we can use
  wherever we want to do async stuff in our app
...
> 📜 Read up on
> [Error Boundaries](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary) if you
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Suspense%20%F0%9F%94%80&e=01%3A%20Simple%20Data-fetching&em=).
