...
- Preloading images to the browsers cache
  - Such images can be useful as fallbacks for when we´re waiting on actual
    images to be fetched which react suspense can help us with
- How we can create an Img component that makes the request for our image to
  have react suspend the image
- How, if our API allows for it, we can fetch the image as soon as we have the
  data we need for that (hopefully at the same time as the data we want to show
  with image) and thus avoiding the fetch waterfall
- Rendering all this as we fetch by also lazy loading our component showing all
  this data + image
